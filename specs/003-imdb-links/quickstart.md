...
4. Re-run the exact same command; confirm `entriesScanned` reflects only rows still lacking `imdb_id` — already-resolved rows from the first run are excluded from the scan and untouched (idempotency, FR-010), verifiable via a temporarily-broken `TMDB_API_KEY` showing no request for them. Any permanent orphans from the first run (`no_tmdb_match` or `api_error`) are rescanned and re-attempted against TMDB here too, by design — a plain re-run is itself the retry mechanism for orphans (see [[contracts/backfill-imdb-ids-script]]'s Non-goals). `entriesScanned: 0` and zero new TMDB calls only hold once no orphans remain at all; with any orphan still present, expect `entriesScanned` equal to the orphan count and a fresh TMDB call per orphan.
