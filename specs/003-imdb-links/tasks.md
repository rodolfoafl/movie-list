...
- [ ] T001 Add nullable `imdb_id` (`text`) column to `movieEntries` in `app/lib/db/schema.ts` (after `releaseYear`, before `watchedAt`, per [data-model.md](./data-model.md) — no index, no default)
