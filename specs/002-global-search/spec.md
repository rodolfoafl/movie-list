...
- **FR-019**: The modal's list set and each list's checked/disabled state MUST NOT be re-fetched or live-updated while the modal remains open; both reflect a snapshot taken at the moment the modal opens (promotes the existing Assumption to a testable requirement). A fetch retry after an initial load failure, before any snapshot has been successfully established, is not considered a re-fetch under FR-019 — FR-019 protects an already-established snapshot from changing while the modal remains open, not the initial attempt to establish one.
