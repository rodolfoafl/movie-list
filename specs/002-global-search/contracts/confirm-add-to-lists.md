...
3. Zip each settled result back with its `listId` before returning: `{ listId, status, reason }`. `addMovieToListWithOutcome` itself has no idea which list it was called for beyond its own `listId` parameter, so this zip step — not a change to `addMovieToListWithOutcome`'s return shape — is what lets the caller pair outcomes with list names.
   ...
   - `settled[i].status === "rejected"` and `settled[i].reason` is a redirect error (session expired mid-fan-out, rethrown by `addMovieToListWithOutcome` — see [[contracts/add-movie-to-list-with-outcome]], [[research]] §8) → rethrown via `unstable_rethrow` here too, propagating out of `confirmAddToLists` rather than being converted to a failure outcome.
   - `settled[i].status === "rejected"` for any other reason (defensive only — not expected in normal operation) → `{ listId: listIds[i], status: "failure", reason: "Não foi possível adicionar, tente novamente." }`.
